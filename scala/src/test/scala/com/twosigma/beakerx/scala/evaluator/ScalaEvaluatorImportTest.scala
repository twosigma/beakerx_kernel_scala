...
class ScalaEvaluatorImportTest {
  ...
  def importHandling(): Unit = {
    ...
    val importSeo = SimpleEvaluationObjectMock.createSeo(importCode)
    val importEvaluate = scalaEvaluator.evaluate(importSeo, importCode)
    importEvaluate.isError shouldBe false

    val classSeo = SimpleEvaluationObjectMock.createSeo(classCode)
  ...
  def scalaStandardTypes(): Unit = {
    ...
    val scalaEvaluator = TestScalaEvaluator.evaluator
    val importSeo = SimpleEvaluationObjectMock.createSeo(code)
