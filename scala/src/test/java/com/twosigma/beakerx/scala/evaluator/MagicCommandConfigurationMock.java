...
import com.twosigma.beakerx.FileServiceMock;
...
import com.twosigma.beakerx.kernel.command.MagicCommandWhichThrowsException;
...
import static com.twosigma.beakerx.kernel.command.ClasspathAddMvnDepsMagicCommandTestInfo.TEST_MVN_CACHE;
...
public class MagicCommandConfigurationMock implements MagicCommandConfiguration {
  ...
  public List<MagicCommandType> createDefaults(KernelFunctionality kernel) {
    ...
    magicCommandTypes.addAll(Lists.newArrayList(
            ...
            new MagicCommandType(ClasspathAddJarMagicCommandInfo.CLASSPATH_ADD_JAR, "<jar path>", new ClasspathAddJarMagicCommand(kernel, MessageCreator.get())),
            new MagicCommandType(ClasspathAddMvnMagicCommand.CLASSPATH_ADD_MVN, "<group name version>",
                    new ClasspathAddMvnMagicCommand(mavenResolverParam, kernel, MessageCreator.get())),
            new MagicCommandType(ClassPathAddMvnCellMagicCommand.CLASSPATH_ADD_MVN_CELL, "<group name version>",
                    new ClassPathAddMvnCellMagicCommand(mavenResolverParam, kernel, MessageCreator.get())),
            addClasspathReset(kernel, fileService),
            addDynamic(kernel),
            addMagicCommandWhichThrowsException(),
            new MagicCommandType(ClasspathShowMagicCommand.CLASSPATH_SHOW, "", new ClasspathShowMagicCommand(kernel, MessageCreator.get())),
            new MagicCommandType(AddStaticImportMagicCommand.ADD_STATIC_IMPORT, "<classpath>", new AddStaticImportMagicCommand(kernel)),
            new MagicCommandType(AddImportMagicCommandInfo.IMPORT, "<classpath>", new AddImportMagicCommand(kernel, MessageCreator.get())),
            new MagicCommandType(UnImportMagicCommand.UNIMPORT, "<classpath>", new UnImportMagicCommand(kernel, MessageCreator.get())),
            new MagicCommandType(TimeLineModeMagicCommand.TIME_LINE, "", new TimeLineModeMagicCommand(kernel, SimpleEvaluationObjectFactory.get())),
            new MagicCommandType(TimeCellModeMagicCommand.TIME_CELL, "", new TimeCellModeMagicCommand(kernel, SimpleEvaluationObjectFactory.get())),
            new MagicCommandType(TimeItLineModeMagicCommand.TIMEIT_LINE, "", new TimeItLineModeMagicCommand(kernel, SimpleEvaluationObjectFactory.get())),
            new MagicCommandType(TimeItCellModeMagicCommand.TIMEIT_CELL, "", new TimeItCellModeMagicCommand(kernel, SimpleEvaluationObjectFactory.get())),
            new MagicCommandType(LoadMagicMagicCommand.LOAD_MAGIC, "", new LoadMagicMagicCommand(kernel, MessageCreator.get())),
  ...
  public MagicCommandType async(KernelFunctionality kernel) {
    return new MagicCommandType(AsyncMagicCommand.ASYNC, "", new AsyncMagicCommand(kernel, SimpleEvaluationObjectFactory.get()));
  }
  ...
  private static MagicCommandType addDynamic(KernelFunctionality kernel) {
    return new MagicCommandType(ClasspathAddDynamicMagicCommand.CLASSPATH_ADD_DYNAMIC, "", new ClasspathAddDynamicMagicCommand(kernel, MessageCreator.get(), SimpleEvaluationObjectFactory.get()));
  }
