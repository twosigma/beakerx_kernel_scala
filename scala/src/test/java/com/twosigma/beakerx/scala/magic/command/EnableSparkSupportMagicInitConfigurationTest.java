...
import com.twosigma.beakerx.scala.ScalaKernelTestFactory;
import com.twosigma.beakerx.scala.evaluator.MagicCommandConfigurationMock;
...
public class EnableSparkSupportMagicInitConfigurationTest {
  ...
  @Before
  public void setUp() {
    this.kernel = ScalaKernelTestFactory.getKernel();
  ...
  public void runSparkMagicCommand() {
    ...
    MagicCommand command = new MagicCommand(new ClassPathAddMvnCellMagicCommand(configurationMock.mavenResolverParam(kernel), kernel, MessageCreator.get()), allCode);
    Code code = Code.createCode(allCode, Collections.singletonList(command), NO_ERRORS, commMsg(), MessageCreator.get(), new SimpleEvaluationObjectFactory());
    //when
